      ...
      switch (player.role) {
        case "striker":
          if (this.teamState.possession) {
            // Position for attack
            targetPos.set(ballPos.x * 0.8, 0, Math.min(ballPos.z + 5, 13));
          } else {
            // Position for pressing
            targetPos.set(ballPos.x * 0.6, 0, ballPos.z * 0.8);
          }
          break;
        case "midfielder":
          targetPos.set(ballPos.x * 0.5, 0, ballPos.z * 0.3);
          break;
        case "defender":
          targetPos.set(ballPos.x * 0.3, 0, Math.max(ballPos.z * 0.2, -12));
          break;
      }
      return targetPos;
    } catch (error) {
