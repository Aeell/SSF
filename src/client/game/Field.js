...
export default class Field {
    ...
    createGround() {
        try {
            ...
            const groundShape = new CANNON.Box(new CANNON.Vec3(this.width/2, this.height/2, this.length/2));
    ...
    createBoundaries() {
        try {
            ...
            const walls = [
                // Left wall
                { size: [wallThickness, wallHeight, this.length], position: [-this.width/2 - wallThickness/2, wallHeight/2, 0] },
                // Right wall
                { size: [wallThickness, wallHeight, this.length], position: [this.width/2 + wallThickness/2, wallHeight/2, 0] },
                // Back wall
                { size: [this.width + wallThickness*2, wallHeight, wallThickness], position: [0, wallHeight/2, -this.length/2 - wallThickness/2] },
                // Front wall
                { size: [this.width + wallThickness*2, wallHeight, wallThickness], position: [0, wallHeight/2, this.length/2 + wallThickness/2] }
            ];
    ...
    createGoals() {
        try {
            ...
            const createGoalMesh = (position) => {
                ...
                const posts = [
                    // Left post
                    { size: [goalThickness, goalHeight, goalThickness], position: [-goalWidth/2, goalHeight/2, 0] },
                    // Right post
                    { size: [goalThickness, goalHeight, goalThickness], position: [goalWidth/2, goalHeight/2, 0] },
                    // Crossbar
                    { size: [goalWidth, goalThickness, goalThickness], position: [0, goalHeight, 0] }
                ];
                ...
                posts.forEach(post => {
                    ...
                    const shape = new CANNON.Box(new CANNON.Vec3(post.size[0]/2, post.size[1]/2, post.size[2]/2));
                    const body = new CANNON.Body({
                        mass: 0,
                        shape: shape,
