...
export class Team {
    ...
    getInitialPosition(role, index) {
        ...
        const zOffset = isHome ? -this.scene.field.getLength()/4 : this.scene.field.getLength()/4;
        ...
        switch(role) {
            ...
            case 'goalkeeper':
                return new THREE.Vector3(
                    0,
                    1,
                    zOffset + (isHome ? -this.scene.field.getLength()/2 + 2 : this.scene.field.getLength()/2 - 2)
                );
            default:
    ...
    updateTeamState() {
        // Check possession
        this.state.hasPossession = this.players.some(player => player.state.hasBall);
    }
